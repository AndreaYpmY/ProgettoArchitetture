...
{
	// int num_cores = omp_get_num_procs();  // Numero di core disponibili
...
{
	VECTOR all_C_alpha = alloc_matrix(n * 3, 1);
...
type hydrophobicity_energy(char *s, MATRIX coords, int n)
{
	...
	// all_c_alpha = alloc_matrix(n*3, 1);		//Si può eliminare
	all_c_alpha = c_alpha(coords, n);

//provare a togliere fuori dal ciclo c_alpha_i/j
...
type electrostatic_energy(char *s, MATRIX coords, int n)
{
	...
	for (int i = 0; i < n; i++)
	{
		...
		for (int j = i + 1; j < n; j++)
		{
			...
			distanza = dist(c_alpha_i, c_alpha_j); // VARIABILE LOCALE QUINDI NON SERVE LA REDUCTION (NON NECESSITA DI PROTEZIONE)
...
/*
void save_to_txt(const char *filename, MATRIX data1, MATRIX data2, int rows, int cols)
